  ...
  return {
    ...
    actions: {
      ...
      PasswordResetPsycho: async (email) => {
        ...
        try {
          const response = await fetch(
            apiUrl + "/generate_reset_token_psychologist",
            requestOptions
          );
      ...
      startConversation: async (userId) => {
        ...
        try {
          ...
          const ides = localStorage.setItem(
            "conversation_id",
        ...
        try {
          ...
          if (!response.ok) {
            if (response.status === 404) {
              ...
              console.error(
                "Error con el fetch del psicólogo:",
                response.statusText
              );
            }
            return null;
      ...
      updatePsico: async (nuevosDatosPsico) => {
        ...
        try {
          const response = await fetch(
            apiUrl + `/psico/${store.psychologist.id}`,
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
          ...
          if (response.ok) {
            ...
            console.error(
              "Error al actualizar los datos del psicólogo:",
              response.statusText
            );
          }
        } catch (error) {
